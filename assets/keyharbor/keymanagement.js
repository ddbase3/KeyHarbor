(function() {
	...
	function init() {
		...
		async function postJson(payload) {
			...
			catch(error) { throw new Error(t('invalid_response', 'Credential service returned an invalid response.')); }
		...
		function createCredentialCard(credential) {
			...
			title.appendChild(textElement('h2', credential.label || t('unnamed_credential', 'Unnamed credential')));
			...
			appendMeta(meta, t('meta_created', 'Created'), formatDate(credential.created_at));
			appendMeta(meta, t('meta_expiration', 'Expiration'), credential.expires_at ? formatDate(credential.expires_at) : t('permanent', 'Permanent'));
			appendMeta(meta, t('meta_mode', 'Mode'), credential.auth_mode === 'hmac' ? 'HMAC' : 'Bearer');
			appendMeta(meta, t('meta_notifications', 'Notifications'), credential.notification_address || t('not_configured', 'Not configured'));
			...
			(Array.isArray(credential.service_ids) ? credential.service_ids : []).forEach((serviceId) => {
				...
				if(!service) {
					...
					chip.title = t('service_unavailable_title', 'This service is not currently available.');
				}
			...
			actions.appendChild(actionButton(t('edit', 'Edit'), 'edit', credential.id, 'keyharbor-button-secondary', isRevoked));
			const rotateButton = actionButton(t('rotate', 'Rotate'), 'rotate', credential.id, 'keyharbor-button-secondary', isRevoked || isExpired);
			if(isExpired) { rotateButton.title = t('rotate_expired_help', 'Extend the expiration before rotating this credential.'); }
			actions.appendChild(rotateButton);
			if(isRevoked) {
				actions.appendChild(actionButton(t('delete', 'Delete'), 'delete', credential.id, 'keyharbor-button-danger', false));
			}
			else {
				actions.appendChild(actionButton(t('revoke', 'Revoke'), 'revoke', credential.id, 'keyharbor-button-danger', false));
			}
		...
		function openEditor(credential) {
			...
			editorTitle.textContent = existing ? t('editor_edit_title', 'Edit credential') : t('editor_create_title', 'Create credential');
			saveButton.textContent = existing ? t('save_changes', 'Save changes') : t('create', 'Create credential');
		...
		function renderServiceOptions(credential) {
			...
			selected.forEach((serviceId) => {
				...
				serviceList.appendChild(serviceOption(serviceId, serviceId, t('service_unavailable_remove', 'Service is not currently available. Uncheck it to remove the grant.'), true, false));
			});

			if(serviceList.children.length === 0) {
				serviceList.appendChild(textElement('p', t('no_services', 'No credential-protected services are currently available.')));
			}
		}
		...
		async function saveEditor() {
			...
			setButtonBusy(saveButton, true, payload.mode === 'create' ? t('creating', 'Creating…') : t('saving', 'Saving…'));
			try {
				...
				setNotice(payload.mode === 'create' ? t('created_success', 'Credential created.') : t('updated_success', 'Credential updated.'), 'success');
			}
		...
		function validateEditor() {
			if(!fields.label.value.trim()) {
				setEditorError(t('validation_label_required', 'Credential label is required.'));
				fields.label.focus();
				return false;
			}
			if(fields.expiresEnabled.checked && !fields.expiresAt.value) {
				setEditorError(t('validation_expiration_required', 'Select an expiration date.'));
				fields.expiresAt.focus();
				return false;
			}
			if(fields.expiresEnabled.checked && !fields.notificationAddress.value.trim()) {
				setEditorError(t('validation_notification_required', 'An expiring credential requires a notification address.'));
				fields.notificationAddress.focus();
				return false;
			}
			if(serviceList.querySelectorAll('[data-service-grant]:checked').length === 0) {
				setEditorError(t('validation_service_required', 'Select at least one credential service.'));
				return false;
			}
			return true;
		}
		...
		function readExpiration() {
			...
			if(!fields.expiresAt.value) { throw new Error(t('validation_expiration_required', 'Select an expiration date.')); }
			const timestamp = Math.floor(new Date(fields.expiresAt.value).getTime() / 1000);
			if(!Number.isFinite(timestamp)) { throw new Error(t('validation_expiration_invalid', 'Expiration date is invalid.')); }
			return timestamp;
		}
		...
		async function rotateCredential(credentialId) {
			...
			if(!credential || !window.confirm(formatText(
				t('confirm_rotate', 'Rotate \"{credential}\"? The current token will stop working immediately.'),
				{ credential: credential.label || t('credential_fallback', 'credential') }
			))) { return; }
			try {
				setNotice(t('rotating', 'Rotating credential…'), 'info');
				...
				setNotice(t('rotated_success', 'Credential rotated. The previous token is invalid.'), 'success');
			}
		...
		async function revokeCredential(credentialId) {
			const credential = credentialById(credentialId);
			if(!credential || !window.confirm(formatText(
				t('confirm_revoke', 'Revoke \"{credential}\"? This action cannot be undone.'),
				{ credential: credential.label || t('credential_fallback', 'credential') }
			))) { return; }
			try {
		...
		async function deleteCredential(credentialId) {
			...
			if(!credential || credential.status !== 'revoked' || !window.confirm(formatText(
				t('confirm_delete', 'Permanently delete \"{credential}\"? This removes the credential and its grants and cannot be undone.'),
				{ credential: credential.label || t('credential_fallback', 'credential') }
			))) { return; }
			try {
		...
		function showToken(token, authenticationMode) {
			...
			tokenModeNote.textContent = hmac
				? t('hmac_usage', 'Use this token with X-BASE3-Timestamp, X-BASE3-Nonce and X-BASE3-Signature. Sign the canonical request with the token secret using HMAC-SHA256.')
				: '';
		...
		async function copyToken() {
			...
			try {
				...
				copyStatus.textContent = t('token_copied', 'Token copied.');
			}
			catch(error) { copyStatus.textContent = t('copy_failed', 'Copy failed. Select the token and copy it manually.'); }
		}
		...
		function updateCreateButtons() {
			root.querySelectorAll('[data-action="create"]').forEach((button) => {
				...
				button.title = state.services.length === 0 ? t('no_services_available', 'No credential services are currently available.') : '';
			});
		}
		...
		function setButtonBusy(button, busy, label) {
			if(busy) {
				...
				button.textContent = label || t('working', 'Working…');
		...
		function responseError(data) {
			...
			return localized || t('request_failed', 'Credential request failed.');
		}
